...
apps/web                   (depends on @licio/shared, @licio/invariants,
                            @licio/ai-governance, @licio/lcap; NEVER
                            @licio/db — enforced by check:workspace-deps.
                            @licio/lcap is used ONLY by the WS-R.15.1
                            offline bundle export/import flows and is loaded
                            as a lazy dynamic-import chunk, so the protocol
                            core never enters the initial bundle)
