...
and the §10.8 four-tier key store + §12.6/§12.7 recovery kit — every primitive a
thin RFC-vector-pinned WebCrypto wrapper (RFC 5869/7748/8032/9180 + an `@hpke/core`
interop vector + `@noble/curves` KATs).  The remaining WS-S plane (3.6c threshold
recovery, the §9 content-addressing + P2P transport — full Helia vetted at 580
transitive packages, so the lighter WebRTC/IPFS-bridge path is the open
maintainer call — the Lamport reducer, sync/rendezvous, UI/media/migration, the
update channel) is the next slice.  See "Implementation roadmap" below.
...
| packages/private-p2p | ~13 (WS-S Private P2P rooms: the canonical DAG-CBOR determinism/reject/bomb suite — P1/P2/P3 + the integer-boundary table + the §27 cap aborts; the strict private-schema accept/reject + WS-G contribution-op parity + the §10.4↔§10.5 envelope-AAD alignment + every §13.2 op-body type; AND the **WS-S.3 crypto foundation** — RFC 5869 HKDF vectors, the two-layer AEAD (AAD-flip/epoch-replay/nonce-uniqueness), Ed25519 KATs cross-validated against `@noble/curves` + the RFC 9180 HPKE interop vector + RFC 7748 X25519, the MLS multi-device/epoch/manifest-fork suite, the four-tier key store + the recovery kit, and the forward-secrecy + fuzz properties — crypto 97% stmts / 92% branch) | node | `pnpm --filter @licio/private-p2p test` |
