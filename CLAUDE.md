...
│           │   ├── server-ingest.ts      --   in-memory ingestion engine: CID-verified CAS +
│           │   │                             per-room acceptance log + idempotency + fork
│           │   │                             detection over the pure ingestRecord (WS-R.12.1a/c);
│           │   │                             server-computed validate() over registered identity
│           │   │                             state (certs/caps/authority keys/revocations, R.12.1b);
│           │   │                             commitBatch (§24.4 ordered, §27.2 graph-guarded)
│           │   ├── routes.ts             --   §29 content-read endpoints (GET by CID; §22.1.1
│           │   │                             status), mounted at /api/lcap/v2 (WS-R.12.4)
