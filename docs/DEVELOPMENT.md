...
| API logs `FATAL … Port 3001 is already in use` and exits 1 | A second `pnpm dev`, or a previous run whose `tsx watch` supervisor outlived its terminal | Free the port **and** kill the watcher — `lsof -ti :3001 \| xargs kill -9`, then kill any lingering `tsx watch src/index.ts`. Killing only the process on the port is not enough: the watcher **respawns** its child, so the port comes straight back. Or leave it running and start the second instance with `PORT=<n>`. (Vite does this for you — it just picks 5174, which is why only the API fails loudly.) |
