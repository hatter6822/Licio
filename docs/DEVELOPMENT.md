...
| `KNOMOSIS_GATEWAY_URL` + `KNOMOSIS_GATEWAY_TOKEN_FILE` group | The WS-L Knomosis gateway uses the deterministic in-memory `FakeKnomosisGateway` (fine for dev; no real substrate). Both must be set together to bind the real `HttpKnomosisGateway` (bearer token read from the file). Independent of this, all WS-L endpoints stay **off** until the `knomosis.cryptoEnabled` / `knomosis.governanceEnabled` runtime-config keys are flipped `true` through the admin surface — both default `false` and fail closed, so a bare `pnpm dev` box has the wallet/gateway/simulation surface disabled (endpoints return `503`) |
