...
| `TM-MFCI` | MFCI incidents by severity | Count and severity distribution of coordination incidents | MFCI service (WS-H.3) | Count by severity | Aggregate only; no individual cases |
| `TM-GWEI` | GWEI cohort disparity | Structural experience parity across defined cohorts | GWEI service (WS-H.5) | Cohort disparity ratio | Min 5 cohorts; none < 100 users |
| `TM-PHI` | PHI steering-risk distribution | Distribution of path-dependency risk scores | PHI service (WS-H.6) | Distribution (percentiles) | Aggregate only; no per-user scores |
| `TM-HPL` | Harassment-protection latency | Time from harassment report to target protection | Safety queue logs (WS-J) | Latency percentiles (p50/p95) | Aggregate; no individual case timing |
| `TM-AOR` | Appeal-overturn rate | Fraction of appeals resulting in reversal | Appeals system (WS-J.1.3) | Ratio (overturned ÷ total) | Min 20 appeals/period |
| `TM-ADR` | Accessibility-defect rate | Accessibility regression defects per release | QA tracking (WS-B/WS-0) | Count per release | Per release |
| `TM-CWV` | Core Web Vitals (LCP, INP, CLS) | Performance at p75 across real users | RUM / field data (WS-P) | p75 per metric | Min 1000 page loads/period |
...
| Metric ID | Metric | Definition | Aggregation method | Guards against |
|---|---|---|---|---|
| `KM-GRANT` | Public-value grant completion | Funded grants producing accepted evidence/context outputs | Completion ratio | Treasury waste |
| `KM-TXCOMP` | Transaction comprehension | User-test success on transaction-preview meaning | Success ratio | Blind signing |
| `KM-TREASTRANS` | Treasury-transparency completeness | Treasury actions with clear proposal/recipient/amount/outcome | Completeness ratio | Dark-money governance |
| `KM-GOVDIV` | Governance diversity | Participation breadth across eligible civic accounts (not wallet wealth) | Diversity index across cohorts | Capture |
| `KM-PROPDISP` | Proposal-dispute rate | Proposals challenged for conflict/fraud/policy | Ratio (disputed ÷ total) | Unaccountable execution |
| `KM-FININC` | Financial-incident rate | Confirmed scams/fraud/mistaken transfers/compromise per active wallet | Rate per active wallet | Unsafe expansion |
| `KM-P2RLEAK` | Pay-to-rank leakage | Measured correlation between payments and ranking after controls | Residual correlation coefficient | Wealth-driven visibility |
| `KM-RECONGAP` | Treasury-reconciliation gap | Divergence between app ledger, Knomosis receipts, and L1 state | Signed difference (target 0) | Must be zero or explained before expansion |
...
```json
{
  ...
  "product_health_metrics": [
    { "metric_id": "TM-CPR", "name": "Constructive-participation rate", "aggregation": "Mean rate (constructive / total)", "privacy_threshold": "Min 100 contributions/period", "cadence": "monthly", "responsible_ws": "WS-K" },
    { "metric_id": "TM-SOR", "name": "Source-open rate", "aggregation": "Ratio (source opens / views)", "privacy_threshold": "Min 1000 views/period", "cadence": "monthly", "responsible_ws": "WS-E" },
    { "metric_id": "TM-EAR", "name": "Evidence-addition rate", "aggregation": "Mean per active thread", "privacy_threshold": "Min 50 threads/period", "cadence": "monthly", "responsible_ws": "WS-F" },
    { "metric_id": "TM-QRR", "name": "Question-resolution rate", "aggregation": "Ratio (answered / asked)", "privacy_threshold": "Min 50 questions/period", "cadence": "monthly", "responsible_ws": "WS-G" },
    { "metric_id": "TM-MERI", "name": "MERI distribution", "aggregation": "Distribution (percentiles)", "privacy_threshold": "Aggregate only; no per-user scores", "cadence": "monthly", "responsible_ws": "WS-H.2" },
    { "metric_id": "TM-SCOI", "name": "SCOI reduction after bridge/synthesis", "aggregation": "Mean delta (pre to post)", "privacy_threshold": "Min 20 bridge events/period", "cadence": "monthly", "responsible_ws": "WS-H.4" },
    { "metric_id": "TM-MFCI", "name": "MFCI incidents by severity", "aggregation": "Count by severity", "privacy_threshold": "Aggregate only; no individual cases", "cadence": "monthly", "responsible_ws": "WS-H.3" },
    { "metric_id": "TM-GWEI", "name": "GWEI cohort disparity", "aggregation": "Cohort disparity ratio", "privacy_threshold": "Min 5 cohorts; none < 100 users", "cadence": "monthly", "responsible_ws": "WS-H.5" },
    { "metric_id": "TM-PHI", "name": "PHI steering-risk distribution", "aggregation": "Distribution (percentiles)", "privacy_threshold": "Aggregate only; no per-user scores", "cadence": "monthly", "responsible_ws": "WS-H.6" },
    { "metric_id": "TM-HPL", "name": "Harassment-protection latency", "aggregation": "Latency percentiles (p50/p95)", "privacy_threshold": "Aggregate; no individual case timing", "cadence": "monthly", "responsible_ws": "WS-J" },
    { "metric_id": "TM-AOR", "name": "Appeal-overturn rate", "aggregation": "Ratio (overturned / total)", "privacy_threshold": "Min 20 appeals/period", "cadence": "monthly", "responsible_ws": "WS-J.1.3" },
    { "metric_id": "TM-ADR", "name": "Accessibility-defect rate", "aggregation": "Count per release", "privacy_threshold": "Per release", "cadence": "per-release", "responsible_ws": "WS-B" },
    { "metric_id": "TM-CWV", "name": "Core Web Vitals (LCP, INP, CLS)", "aggregation": "p75 per metric", "privacy_threshold": "Min 1000 page loads/period", "cadence": "monthly", "responsible_ws": "WS-P" }
  ],
  "knomosis_metrics": [
    { "metric_id": "KM-GRANT", "name": "Public-value grant completion", "aggregation": "Completion ratio", "guards_against": "Treasury waste", "expansion_blocking": false },
    { "metric_id": "KM-TXCOMP", "name": "Transaction comprehension", "aggregation": "Success ratio", "guards_against": "Blind signing", "expansion_blocking": false },
    { "metric_id": "KM-TREASTRANS", "name": "Treasury-transparency completeness", "aggregation": "Completeness ratio", "guards_against": "Dark-money governance", "expansion_blocking": false },
    { "metric_id": "KM-GOVDIV", "name": "Governance diversity", "aggregation": "Diversity index across cohorts", "guards_against": "Capture", "expansion_blocking": false },
    { "metric_id": "KM-PROPDISP", "name": "Proposal-dispute rate", "aggregation": "Ratio (disputed / total)", "guards_against": "Unaccountable execution", "expansion_blocking": false },
    { "metric_id": "KM-FININC", "name": "Financial-incident rate", "aggregation": "Rate per active wallet", "guards_against": "Unsafe expansion", "expansion_blocking": false },
    { "metric_id": "KM-P2RLEAK", "name": "Pay-to-rank leakage", "aggregation": "Residual correlation coefficient", "guards_against": "Wealth-driven visibility", "expansion_blocking": true },
    { "metric_id": "KM-RECONGAP", "name": "Treasury-reconciliation gap", "aggregation": "Signed difference (target 0)", "guards_against": "Reconciliation divergence", "expansion_blocking": true }
  ],
  "anti_metrics": [
