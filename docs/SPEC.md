...
**Model candidacy is a member power** (2026-07-21 revision): any governance-eligible room member (the elected seat-holder always counts as a member for this purpose) may **propose a community governance model** — a content-addressed, hash-pinned, reproducible bundle (a moderation/governance prompt + config, optionally selecting **revision-pinned huggingface.co models** for the room's moderation and/or adjudication roles) — together with **the in-room prompt** that conditions it for the room's law-pack and norms. Every proposal must clear the **platform evaluation/red-team admission gate** (§24.2, §24.6) before it is eligible to be voted on, and adoption **requires a Knomosis governance vote of the room's members** — the same electorate that elects the steward.
