...
**The challenge policy.** Challenging is deliberately rationed so adjudication capacity (a governed-LLM verdict per debate) goes to genuine disputes — without ever gating the *ability* to challenge:

- **Capacity.** Each account holds a bounded number of challenges in flight (arenas pre-verdict — open, locked, or awaiting the verdict; the slot frees when the verdict lands, not when the override window closes). The floor is **1 for every account and is never KYC-gated** (content participation never is, Section 19.1's spirit); reviewer-verified KYC standing adds a capacity bonus (default +2), and **adjudicated challenger wins** — `decided_by` AI or steward; an uncontested concession credits nothing — earn further tiers (default one per three wins), **deduped per opponent account** (default cap 2 per opponent, the sibling-subdomain rule of standing: a sockpuppet ring cannot mint tiers) and gated on a minimum adjudicated win rate (default 50%). Ceilings: 4 without KYC, 8 with. A trailing-24h open budget (default 5) backstops velocity. Every number is steward-tunable runtime config; all standing state is **derived from arena rows**, never a mutable counter.
- **Withdrawal pricing.** Withdrawing inside a short grace window (default 5 minutes, and only while the incumbent has never engaged) costs nothing and consumes nothing — the misclick exit. Any later withdrawal starts an escalating cooldown (defaults 2h / 24h / 72h by rank within a trailing 30-day window) and, past a free allowance (default one per window), temporarily reduces capacity — never below the floor of 1. The pricing sits deliberately **below the cost of losing** (the correction tagged `incorrect` plus the win-rate hit), so a challenger who realizes they are wrong still exits before the adjudicator spends a verdict. Deleting one's own correction mid-debate is a withdrawal and carries the same consequences.
- **Once per target per version.** An account gets one concluded challenge (resolved, or non-grace-withdrawn) per target per content version; a material edit of the target re-opens it. Self-challenges are refused outright at create (they can be neither a genuine adversarial adjudication nor allowed to park the target's single live-arena slot).
- **The settled threshold.** A target that accumulates the configured number of adjudicated `upheld` defenses since its last material edit (default 3 — necessarily distinct challengers, given once-per-target) is **settled**: further challenges are refused. Settling closes only the *debate* lane — Section 18.4 reports and the platform moderation floor are untouched — and is reversible: the target's home-room stewards (the Commons included — the same steward arm that already holds the stronger verdict-override power) or a platform admin under the per-session MFA bar may **unsettle** when genuinely new evidence emerges, whereupon the next upheld defense re-settles immediately; a material edit clears both the `validated` posture and the settled mark (the defended text is no longer the served text). Settled is stored as a separate mark (`settled_at`), never a new dispute status — so it reads `validated` on the ranking and search surfaces, which are untouched by construction. The card surfaces the settled distinction through an optional `dispute_settled` wire marker (emitted only when true — a new optional key, not a new enum value, so a pre-settle cached client keeps parsing every not-yet-settled response), which renders the terminal "Settled" badge in place of "Validated".
- **Standing surface.** `GET /v1/challenge-standing` serves the caller their **own** quota, cooldown, and an optional per-target probe, so the correction composer explains *before* filing why a challenge cannot be opened. No other account's record is ever served: capacity is private plumbing, never a leaderboard (Section 15.6).
