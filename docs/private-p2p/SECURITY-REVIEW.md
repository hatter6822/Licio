...
| **Rendezvous presence flood (§27, DoS)** | A member floods a room blind id with opaque presence records to crowd honest peers out of the `poll` window | The §8.2 server-blind allowlist exposes NO stable identity / insertion-order column, so a true per-announcer cap is **infeasible** without breaking server-blindness; the available defenses are the per-room cap, the global IP-free rate limit (bounds announce rate), the member-only rendezvous key, and the insertion-ordered poll (favours incumbents) | Documented limitation: presence flood-fairness is best-effort under server-blindness; the rendezvous key is the trust boundary |
