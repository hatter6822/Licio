...
7. **Server-hosted rooms only (room-class applicability).** WS-M's server-side governance and treasury surface — proposals, voting, execution, treasuries, law-packs, action budgets, delegation, anti-capture — applies **only to server-hosted rooms** (`public_server` and `restricted_server`; WS-Q / PRIVATE_SPEC §4). A `private_p2p` room (WS-S) has `authority_model = 'room_keys'` (coherence: `storage='p2p' ⇒ authority='room_keys'`): authority lives entirely in the room's MLS key holders, not the platform, so it has **no** server treasury, no server-held membership/proposal/vote/balance state, and no WS-M governance mode. In-room governance (membership, role/capability grants) is performed **locally** via MLS epoch operations (WS-S), and no platform role or emergency key can act on it (§11.4). This is by-design room-class scope, not a reduction of WS-M's server-side surface.
...
> **Room-class scope.** All proposal, voting, execution, delegation, and anti-capture mechanisms in WS-M.4 are **server-side** and apply to server-hosted rooms only (`public_server` / `restricted_server`). For a `private_p2p` room (WS-S, `authority_model='room_keys'`), membership and role/capability changes are MLS-local epoch operations performed by room key holders, never WS-M server-side proposals or votes.
...
| WS-S (private P2P rooms / room-class model) | Room-class applicability boundary: WS-M governance/treasury = server-hosted rooms only; `private_p2p` self-governs via room keys (`authority_model='room_keys'`) | Scope boundary -- no shared code; WS-M endpoints/treasuries never apply to `private_p2p` |
| WS-R (LCAP offline availability) | -- | Orthogonal -- WS-M creates/reconciles no offline governance or treasury artifact; no interaction |
