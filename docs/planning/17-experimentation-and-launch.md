...
Each gate is green/amber/red with the underlying metric values and is consumed by the milestone gate checklists in `00-index.md` and by the go/no-go board (WS-P.3.3a). These phase gates evaluate **core-product** (server-hosted) health; **WS-R and WS-S are launch-relevant** (the 2026-06 maintainer decisions): WS-R is P1, and **WS-S private P2P rooms (E2EE) are LAUNCH-BLOCKING and ship at the same time as the rest of the product**. Each still carries its own launch gate + fail-closed kill switch (WS-R: `docs/OFFLINE_SPEC.md` §36 acceptance suite; WS-S: `docs/PRIVATE_SPEC.md` §29 launch checklist), but the **WS-S §29 gate is FOLDED INTO the GA criteria** (its E2EE-privacy, two-browser convergence, and update-channel guarantees are part of the GA bar) — it is not a separate post-M3 input. (Because `private_p2p` content is structurally unmeasurable, WS-S contributes to the launch decision through its own §29 acceptance suite + security review, not through the server-hosted health metrics above.)
...
**Extension note:** WS-R (`docs/planning/19-decentralized-data-plane.md`) and WS-S (`docs/planning/19-decentralized-data-plane.md`) are downstream, non-blocking consumers of the WS-P.2.2a i18n framework (their mandatory locale-ready disclosure/label copy) and are structurally excluded from WS-P metrics/experiments/transparency-content scope; they are not WS-P dependencies.
