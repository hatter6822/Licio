...
- For a request targeting a `private_p2p` room, the only producible artifacts are the directory stub + blind rendezvous records + the account's Licio-service identity; the workflow surfaces an honest "cannot produce content/keys/member list" determination rather than an empty or misleading production.
...
**Dependencies:** WS-N.2.1a/b (case + legal-hold), WS-N.2.1d (retention/hold), legal-counsel role (WS-A.2), WS-N.2.2d (privacy boundary on producible fields), WS-S (private-room non-storage contract — §8/§11.4 non-producibility), WS-R (offline-reconciliation content scope).
