...
**Description:** Add the defensive guards: in `apps/api/src/ingestion/submission.ts`, **before** auto-join/visibility derivation, reject `room.storageMode === 'p2p'` with `409 p2p_room_requires_client_sync`; in the forum contribution routes/services, reject p2p thread/room ids (defense in depth — p2p ids should never exist server-side); in the upload path, refuse to attach uploads to p2p content and hide server-upload UI inside p2p rooms. `GET /v1/rooms/:id/feed` for a p2p room returns the UNKNOWN-ROOM 404, byte-identical to a room that does not exist (a distinctive code would confirm an `unlisted` room's existence); `GET /v1/search` never returns p2p content.
