...
**Status:** SHIPPED (three additional plugins on the proven Nearby pattern, same `CourierTransport` adapter): `WifiDirectCourierPlugin.java` (Wi-Fi Direct, netsim-verifiable), `BluetoothCourierPlugin.java` (Classic RFCOMM + BLE-GATT fallback, netsim-verifiable), `UsbCourierPlugin.java` (USB accessory mode — physical-OTG-only; there is no emulated USB bus), all compiled into the debug APK as dumb byte pipes (frames re-verified against CIDs/COSE on the TS side, public-only, 64 MiB frame bound).
...
**Status:** SHIPPED + RE-VERIFIED on emulated radios: two headless Android emulators sharing the netsim virtual radio bus exchange a Nearby payload offline (`apps/courier/scripts/radio-e2e.sh` + `NearbyConnectionsRadioTest`); re-run green with the rebuilt APK carrying the three new WS-R.15.4d plugins. Field confirmation on PHYSICAL phones is the only remaining hardware-gated step.
...
**Status:** SHIPPED + wired to REAL takedown state (Gate-19): `lcap_block_provenance` (migration `0046`) maps `block_cid → (target_type,target_id)`; `apps/api/src/lcap/takedown-oracle.ts` (`DrizzleTakedownOracle`, fail-closed — a thrown query is a halt) reads the live status via the single `takedownInForce` rule; `publisher.ts` (`LcapPublicPublisher`) re-checks it at PUBLISH and republish behind `assertPublicGatewayEligible`/`decideBlockPublish`; the env-gated `POST /api/lcap/v2/public-bridge/{publish,republish}` route is the real caller (`@licio/lcap-p2p` is now an `apps/api` dep). A WS-J takedown halts further republication; the decision is auditable.
