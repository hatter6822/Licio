...
This plan decomposes the Licio specification into 21 planning documents housed in `docs/planning/` covering 22 workstreams: the 21 dependency-ordered workstreams (WS-0, A-T) plus the cross-cutting **WS-U** AI-governed-rooms redesign (`22-ai-governed-rooms.md`) — with **WS-R and WS-S unified into the single `19-decentralized-data-plane.md`** (the Decentralized Data Plane upgrade), which supersedes the former `19-offline-content-availability.md` and `20-private-p2p-rooms.md`. Each document is independently actionable, dependency-ordered, and composed of **~994 atomic tasks** targeting 0.5-2 engineering days each (WS-0–WS-T contribute ~945; WS-U adds 49 cards across six staged PRs, decomposed in §U.7 Part II — doctrine-first, no runtime code until ratification). Every task carries a unique ID, a spec reference (`Ref:`), a description, measurable acceptance criteria, testing requirements, and explicit dependencies; data-bearing tasks include Drizzle/zod schemas and API request/response shapes. Tasks are independently reviewable, testable, and reversible per Section 30.8. The plan follows the spec's milestone structure (M0-M6), workstream labels (0, A-U), and the critical-path ordering from Section 30.2. WS-R and WS-S derive from the standalone `docs/OFFLINE_SPEC.md` and `docs/PRIVATE_SPEC.md` specifications (not docs/SPEC.md) and are now documented together in `19-decentralized-data-plane.md` (Part I / Part II); **WS-R was elevated to launch-relevant (P1) by the 2026-06 maintainer decision that makes the native-courier and browser-P2P/WebTransport/IPFS transports first-class, in-scope deliverables**, and **WS-S private P2P rooms (E2EE) are launch-blocking by the 2026-06 maintainer decision — they go live at the same time as the rest of the Licio product (their §29 launch gates + completeness are part of the GA bar, not a post-M3 add-on)**.

**Revision history:**
- **v4.9** — **Refreshed `15-compliance.md` (WS-N) to bind to the shipped WS-L/WS-M compliance seams and the §19.1 identity-free doctrine (2026-07 audit).** The document predated the shipped WS-L/WS-M runtime: WS-N.1.1b's IP-geolocation region detection contradicted SPEC §19.1 (the application never reads the client network address — statically enforced by `no-client-address.test.ts` — and performs no geo-IP lookup of any kind) and is re-specified as **identity-free region resolution** (verified declared region → account-locale subtag → `unknown`, fail-closed; WS-N.1.1f becomes the primary region-*declaration* flow, and verification strength is the sole anti-circumvention control). WS-N.1.1c/2.2a/2.2b/2.2c are re-framed as implementations of the shipped fail-closed `CompliancePort` (`apps/api/src/knomosis/ports.ts`), driving the reserved `payment_compliance_state`/`grant_review_state` values; the `JurisdictionFeaturePolicy` `feature_flags` sub-shape is aligned to the ratified `JURISDICTION_MATRIX.md` closed six-value cell vocabulary (booleans could not express the shipped simulated/testnet modes); age gates are re-expressed in `AGE_BANDS` terms (no DOB exists to evaluate); the registered `compliance.financial.case.created` / `jurisdiction.feature.disabled` topics, the WS-E.1.4 `RetentionOverrides` hook, the isolation-BFS privacy boundary, the shared audit-chain helpers, and the WS-D erasure/legal-hold interplay are now explicit. Two cards added: **WS-N.2.1c-2** (compliance-reviewer + counsel roles — none exist in platform RBAC today) and **WS-N.2.2e** (wallet risk assessment — the shipped `wallet_risk_state` seam had no owning card). All cross-workstream WS-N IDs preserved verbatim. Net +2 cards (WS-N 27→29 → WS-0–WS-T ~945, grand total ~994); also corrected the summary-table Total row, which had not been reconciled since v4.5 (~971 → ~994). No spec change.
- **v4.8** — **Decomposed nine bundled cards in `19-decentralized-data-plane.md` into smaller, independently-testable sub-cards** (the §30.8 one-deliverable-per-card rule applied to the cards whose titles still bundled multiple deliverables). WS-R: `9.2 → 9.2a/9.2b` (Merkle tree | checkpoint record), `9.3 → 9.3a/9.3b` (inclusion | consistency proof), `11.3 → 11.3a/11.3b` (`lcap_v2` schema | §23.2 durability layer), `14.1 → 14.1a/14.1b` (resource caps | malicious-graph detectors), `15.1 → 15.1a/15.1b` (bundle export | import). WS-S: `8.1 → 8.1a/8.1b` (media validation/strip | encrypt/manifest), `3.6 → 3.6a/3.6b/3.6c` (key store/tiers | recovery kit | threshold recovery), `9.2 → 9.2a/9.2b` (migration wizard/destination | import modes), `10.2 → 10.2a/10.2b` (transparency verify | SW-pin/room-lock). Each sub-card carries expanded implementation detail (concrete file paths, signatures, schema fields, exact test cases); the decomposition surfaced one previously-hidden dependency (threshold recovery `WS-S.3.6c → WS-S.5.1`). The dependency graph, milestone-gate `Cards` columns, and task counts were reconciled. Net +10 cards (WS-R 99→104, WS-S 62→67, total 161→171 → WS-0–WS-T ~943, grand total ~992). No spec change.
- **v4.7** — **Unified the two decentralization extension workstreams into one planning document.** Merged the former `19-offline-content-availability.md` (WS-R, 99 cards) and `20-private-p2p-rooms.md` (WS-S, 62 cards) into the new **`19-decentralized-data-plane.md`** — *The Decentralized Data Plane — Offline Content Availability (WS-R / LCAP v0.2) and Private P2P Rooms (WS-S / E2EE)* — and deleted the two former documents. The merge is framing-only: all 161 `WS-R.*`/`WS-S.*` card IDs, spec references, acceptance criteria, and testing notes are preserved **verbatim** (Part I = WS-R, Part II = WS-S), and the two source specs (`docs/OFFLINE_SPEC.md`, `docs/PRIVATE_SPEC.md`) are unchanged. The unified document adds a shared super-frame (the two-planes thesis; the **crypto-suite-separation & key-non-sharing** doctrine — ES256 vs Ed25519/MLS/HPKE; the shared **P2P dependency-isolation** pattern across `@licio/lcap-p2p` and `packages/private-p2p`; the shared doctrine gates; and the **single composition seam** where LCAP carries only WS-S ciphertext, WS-R.16.1 ↔ WS-S.6.5) and a unified back-matter (one cross-plane dependency graph, one phase/wave plan, one merged milestone-gate table, one combined definition of done). Document count 22 → 21 (workstream count unchanged at 22; WS-R and WS-S now share one file); cross-references in `00-index.md`, `16`/`17`/`18`, and `CLAUDE.md`/`AGENTS.md` retargeted. No card-count change (161).
- **v4.6** — **Elevated WS-R's transport posture (maintainer decision, 2026-06).** The native Android courier and the WebTransport/WebRTC/IPFS browser bridges — previously deferred (prototype-only / documentation-only) in `19-offline-content-availability.md` and `docs/OFFLINE_SPEC.md` — are now **first-class, in-scope v0.2 transports**, fully decomposed and gated. WS-R's priority is raised **P3 → P1**, its wave **10 → 8**, and its card count **88 → 99** (+11): the courier becomes a six-card Capacitor sub-area (WS-R.15.4a–f), and the bridges become WS-R.15.5 (WebTransport), WS-R.15.6a/b (WebRTC + NAT/privacy), WS-R.15.7a/b (browser Helia public-block bridge + review gate), WS-R.15.8 (transport dependency-budget + schema-egress gate), and WS-R.15.9 (transport simulation/interop). Doctrine is preserved: every transport reuses the same packs / single `validate` / lane scheduler (no parallel trust path), correctness never depends on any single transport, no peer IP/multiaddr/radio identifier enters any LCAP schema, the heavier WebRTC/Helia deps are confined to the code-split `@licio/lcap-p2p` workspace package (the `apps/web` `<15` direct-dep budget and < 200 KB initial-bundle gate hold) and Capacitor to an `apps/courier` native shell, and IPFS publishes public blocks only behind a required review gate. `docs/OFFLINE_SPEC.md` §22.5/§22.6/§22.7/§31/§31.1/§35/§36/§37/§38/§39 were updated in lockstep so the spec and plan stay non-contradictory. Net +11 cards (WS-R 88→99 → WS-0–WS-T ~922→~933, total ~971→~982).
- **v4.5** — Added **WS-U (AI-governed rooms)** as `22-ai-governed-rooms.md`: the maintainer's binding cross-cutting redesign of AI's role. Inverts the SPEC §24.1 "AI never autonomously spends/approves/sanctions" posture into **bounded autonomy** for *in-room* governance while keeping it intact at the *platform* layer. Establishes the **three-layer authority model** (room sovereignty → Knomosis-bounded AI agent → non-overridable platform legal floor), the **elected room steward** (creator first, then a yearly Knomosis election) with **exactly two powers** (propose a community-approved, member-downloadable AI model; propose its prompt — both member-ratified by Knomosis vote), and the **in-room AI agent** (moderation, treasury management within kernel-enforced bounds, unbiased lawmaking facilitation; holds no keys; sandboxed/capability-scoped; transparent). Amends SPEC §16.3/§16.5/§16.6/§17.1/§17.3.3/§17.4/§17.5/§17.6/§24.1/§24.2/§24.5/§24.6 and the `docs/policy/` register (`README.md`, `STEWARD_ROLES.md` 1.1.0, `MODERATION_TAXONOMY.md` 1.1.0). **Re-scopes WS-K** (the shipped AI-model-governance platform becomes the platform-side evaluation/transparency/guard substrate for community models) and amends **WS-J** (the platform legal floor + appeals of last resort), **WS-L** (Lex bounds + a non-key-holding agent submitter), and **WS-M** (elected steward + Knomosis-vote model/prompt approval + AI-executed treasury within bounds). The **pay-to-rank firewall** (crypto flag, consumer-router Knomosis refusal, financial denylist, wallet↔ranking isolation, governance-only standing read) and **fail-closed crypto** are preserved in full. Doctrine-first (Stage 0); runtime code lands in the six staged PRs of §U.7, now decomposed into **49 atomic cards** (Part II of `22-ai-governed-rooms.md`) with authoritative Drizzle/zod shapes for the seven new `knomosis`-bounded-context entities, the WS-D.3.2 isolation-walk extension, the WS-K registry/harness/guard/monitor reuse, and the §15.4 moderation-port parity.
...
| `15-compliance.md` | WS-N | M5 | P4-5 | 8 | 29 | Jurisdiction engine (identity-free region resolution), financial compliance, support |
...
| `19-decentralized-data-plane.md` (Part I) | WS-R | M3+ (elevated) | P1 | 8 | 104 | LCAP v0.2 delay-tolerant sync: deterministic CBOR/CID/COSE, detached-proof trust plane, lane scheduler, checkpoints/witnesses, liveness, and the **first-class transports** — manual bundle/QR/relay **+ WebTransport, WebRTC P2P, browser-IPFS public bridge, and native Capacitor courier** (elevated 2026-06) |
| `19-decentralized-data-plane.md` (Part II) | WS-S | M3+ (launch-blocking) | P1 | 11 | 67 | E2EE member-hosted private rooms (**launch-blocking — ships with the product, 2026-06**): server non-storage contract, MLS/HPKE/Ed25519, private Helia/libp2p, Lamport-ordered op-log reducer, blind rendezvous, update-channel transparency |
...
WS-0 (Repository foundation) ── 01-repository-foundation.md
 ...
 ├── WS-R (Offline availability / LCAP v0.2; P1, elevated 2026-06 — first-class native courier + WebTransport/WebRTC/IPFS transports) [from OFFLINE_SPEC; depends on WS-C/D/E/F/G/Q — all complete] ── 19-decentralized-data-plane.md (Part I)
 ├── WS-S (Private P2P rooms / E2EE) [extension of PRIVATE_SPEC; depends on WS-C/D/G/Q/O; optional LCAP-pack reuse from WS-R] ── 19-decentralized-data-plane.md (Part II)
...
### Offline content availability (LCAP) — WS-R (elevated to P1 / Wave 8, 2026-06; was Wave 10; post-WS-Q)

- WS-R (LCAP v0.2: deterministic CBOR/CID/COSE foundations → identity/capabilities/revocations → event/block/pack model → lane scheduler → sync/reconciliation/trust projection → room logs/checkpoints/witnesses → liveness/storage → server ingestion → conflict/privacy/DoS controls → **the elevated first-class transports: manual bundle/QR/relay + WebTransport, WebRTC P2P, browser-IPFS public bridge, and native Capacitor courier** → client trust labels → simulation/acceptance) -- `19-decentralized-data-plane.md` (Part I). Net-new `packages/lcap` + the code-split `@licio/lcap-p2p` (WebRTC/Helia, off the web `<15` budget + initial bundle) + `apps/courier` (Capacitor native shell) + `apps/{web,api}/src/lcap` + `lcap_*` DB tables; touches the running app only at the SW/IndexedDB/doctrine-gate/native-shell seams. Foundation sub-area WS-R.0 gates everything; the elevated transports ride the validated core (no parallel trust path; correctness independent of any single transport). Net 104 cards (99 original + 5 decomposition sub-cards).

### Wave 11 (extension; parallel with WS-R): Private P2P rooms (E2EE)

- WS-S (private P2P rooms: room-class model + server non-storage gates → private schemas/canonical encoding → MLS/HPKE/Ed25519/HKDF crypto → private Helia/libp2p profile → op-log + deterministic reducer → sync/blind-rendezvous → private-room UI → media → migration → update-channel transparency → audit/launch) -- `19-decentralized-data-plane.md` (Part II). New `packages/private-p2p` + lazily code-split `apps/web/src/private-p2p` + `private_room_stubs`/`private_rendezvous_records` tables + defensive guards on existing server surfaces. WS-S.1 (server non-storage gates) is landable first, independent of the crypto/P2P stack.
...
3. **WS-R in full** (elevated to P1 / Wave 8, 2026-06) — the content-addressed availability substrate. The core is WebCrypto-native and dependency-free with the shortest path to visible value (offline outbox + honest trust labels by Phase 1); the elevated WebRTC/IPFS transports add reviewed deps confined to the code-split `@licio/lcap-p2p` package (off the web `<15` budget + initial bundle) and the courier to a Capacitor native shell. Its pack/CAR + lane scheduler + liveness UX then make WS-S cheaper.
4. **WS-S core** (Wave 11) — the E2EE plane. Can overlap WS-R once S.1/S.2 and the library selection are done; with two teams it runs concurrently (WS-S is the long pole, ~18–24 wks), with one team it follows WS-R.
