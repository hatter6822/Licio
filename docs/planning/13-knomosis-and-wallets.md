...
> (WS-U Stages 4-5). WS-U's runtime Stages 1-4 and 5-core have since shipped — the
> `@licio/governance` proof-carrying kernel, the bounded moderation/treasury agent, and the
> `/v1/rooms/*` governance routes — so this WS-L submitter plugs into an *existing* bounded
> agent runtime rather than a planned one (see `docs/governance/README.md`); the WS-L additions
> themselves remain future work. Every existing WS-L invariant is **inherited unchanged and is what makes the
