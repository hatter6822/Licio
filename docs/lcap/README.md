...
| WS-R.15 — transport profiles | 15.1a, 15.1b, 15.3 | **Bundle export/import + relay core shipped** — the **WS-R.15.1a/b offline `.licio-bundle` export+import** run CLIENT-LOCAL (`apps/web/src/lcap/bundle-{export,import}.ts` + `OfflineBundlePanel` + the `/profile/offline` route): the real @licio/lcap pack writer/reader + §27.1 caps + quarantine in the browser, loaded as a LAZY dynamic-import chunk (initial bundle untouched); export gathers a room's record→proof→block closure + the §26.2 disclosure-before-file + a generic high-risk filename; import has typed rejection + a pre-render summary + missing-dep quarantine + an INTEGRITY-ONLY commit (nothing rendered before trust projection). The **WS-R.15.3 relay decision core** (`apps/api/src/lcap/relay.ts` `LcapRelay`: store/serve by CID + receipts + §27.3 quotas + private-content refusal + the structural "cannot accept"). The rest is client-UI / native / external-dep — 15.5 (WebTransport) = web UI; 15.4a-f (native Android courier) = the new `apps/courier`; 15.2/15.6/15.7 (QR, WebRTC, IPFS) = new deps |
...
decision core (WS-R.15.3), the client surface (WS-R.17 — honest trust badges,
operational modes, offline-state surfaces), and the client-local offline bundle
export/import (WS-R.15.1a/b) are complete.  The remaining cards are predominantly
**client-UI / native-shell / external-dependency / WS-S-blocked**: WebTransport
(WS-R.15.5), the native Android courier (WS-R.15.4, a new `apps/courier` Capacitor
