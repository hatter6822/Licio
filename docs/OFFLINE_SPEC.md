...
LCAP MUST fit Licio's existing budgets (CLAUDE.md): `apps/web` < 15 and `apps/api` < 20 direct production dependencies, the measured initial-JS budget in `scripts/check-bundle-size.ts` (216 KB gz — re-based from 200 when the lazy-plane chunks were found to be riding the first paint, which the old figure excluded by file name; the real first-paint download FELL from ~433 KB to 214 KB), and the `check:workspace-deps` boundaries.
...
- **Zero-dependency core — with one deliberate, scoped exception.** The codec/CID/COSE half of `packages/lcap` MUST carry **zero** runtime npm dependencies, and does: LDC, the CID computation and the COSE_Sign1 `Sig_structure` are hand-rolled against WebCrypto exactly as the two bullets above require. The SCHEMA half carries `zod`, and that is intended: zod at every trust boundary is an absolute project convention (CLAUDE.md), and an LCAP record arriving over a transport is precisely the boundary that most needs it — hand-rolling validation there to preserve a dependency count would trade a real guarantee for a cosmetic one. `zod` is already a direct dependency of `shared`, `web` and `api`, so it adds no new supply-chain surface, and the package is `workspace:*` (excluded from the `apps/web` direct-dep count) either way. The web LCAP module (`apps/web/src/lcap/`) MUST be **code-split** and loaded only when sync/bundle features are used, so the initial-load bundle-size gate is unaffected.
