...
| `GET /v1/threads` | Thread directory (the public `/threads` tab): a keyset page of conversation summaries (title + contribution count + state), most recent first. Walks the store keyset in bounded batches until a full VISIBLE page accumulates — `threadReadableToUser` removes hidden-story, private/restricted-room, and moderation-removed threads on the optional session, so no fetch prefix strands a readable conversation and a filtered row never stalls the walk (the rooms-directory pattern); page size is `roomPageSize` |
