...
| `apps/web/src/components/comments/` | Inline one-layer comment section + the shared recursive `CommentNode` (depth-capped per view, "continue"/"show more" links onward) and `CommentParts` (header/media/composer), reused by the dedicated comment page (`routes/-pages/story-comments.tsx`, route `stories.$storyId_.comments.tsx`) |
