...
- **WS-E** — Event pipeline and PWAtt: strict event schemas with a single topic registry (14 core + 18 flagged Knomosis topics in a separate bounded context), a hardened attention-ingestion boundary (auth, ownership, two-layer replay protection, fail-closed sliding-window rate limits, server-side privacy enforcement), a retention-tier-partitioned PostgreSQL event store with scheduled retention/anonymization sweeps and a compliance audit query, a rebuildable Redis real-time layer (HyperLogLog uniques), a consumer router enforcing the pay-to-rank firewall, and the PWAtt scoring engine — v0 in CI-verified shadow mode (zero distribution power) with anti-signal detection (coordinated bursts, source-free accusations, harassment-cascade freezes), the private per-user Signal Ledger with plain-language explanations, and v1 saturation curves, guardrailed weight profiles, and separate nonnegative penalties ([`docs/events/README.md`](docs/events/README.md)).
