...
import { commentPage } from '../forum/comments.js';
...
const commentsFor = async (threadId: string, _storyId: string, viewerId: string | null) =>
  (
...
describe('WS-J.1.2 block enforcement', () => {
  it('rejects a reply from a blocked user (interaction_blocked) — bilaterally', async () => {
    ...
    const parent = await post(A, contributionBody('comment', threadId));
    ...
    const reply = await post(
      B,
      contributionBody('comment', threadId, { parentId: parent.contribution.contributionId }),
    );
    ...
    const bQuestion = await post(B, contributionBody('comment', neutral.threadId));
    if (!bQuestion.ok) throw new Error('B question failed');
    const aReply = await post(
      A,
      contributionBody('comment', neutral.threadId, {
  ...
  it('#11 rejects a TOP-LEVEL post from a user blocked by the story owner', async () => {
    ...
    const top = await post(B, contributionBody('comment', threadId));
    ...
    const own = await post(A, contributionBody('comment', threadId));
...
describe('WS-J.1.2 viewing filter (thread reads)', () => {
  it('hides a blocked/muted author content from the viewer, bilaterally', async () => {
    const { threadId, storyId } = await seedThread(fixture, { submittedBy: A });
    const aq = await post(A, contributionBody('comment', threadId));
    const bq = await post(B, contributionBody('comment', threadId));
    ...
    const aView = await commentsFor(threadId, storyId, A);
    const aIds = aView.map((c) => c.contribution_id);
    ...
    const bView = await commentsFor(threadId, storyId, B);
    const bIds = bView.map((c) => c.contribution_id);
    ...
    const anon = await commentsFor(threadId, storyId, null);
  ...
  it('a mute hides the muted author content one-directionally', async () => {
    ...
    const aView = await commentsFor(threadId, storyId, A);
    expect(aView.map((c) => c.contribution_id)).not.toContain(bq.contribution.contributionId);
    // B is unaffected — sees their own content.
    const bView = await commentsFor(threadId, storyId, B);
    expect(bView.map((c) => c.contribution_id)).toContain(bq.contribution.contributionId);
  });
});
