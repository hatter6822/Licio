...
// §30.5). Post-lift role: `rankFrontPageV0` orders through `@licio/ranking`'s
// `chronologicalOrder` — the WS-I SAFE FALLBACK's own ordering (freshness-only)
// — and this guard is what makes that ordering provably score-blind: it rejects
// an invariant output when its shadow_mode flag is set OR when it is a PWAtt
// output AT ALL (belt and braces: the fallback must ignore every PWAtt value
// even post-lift, so engaging the kill switch instantly restores the pre-lift
// posture). Sharing the sort is load-bearing rather than tidy — `rankFrontPageV0`
// is what the shadow-equivalence proof runs, and a proof over a sort production
// does not use would establish nothing about production. The WS-I ranked
