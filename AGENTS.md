...
|-----------|-----------|-------------|-----------------|
| apps/web | ~136 unit + 8 E2E (6 frontend-only + 2 BFF-in-the-loop specs; incl. the WS-J report flow, the notice-inbox appeal affordance, safety controls, the moderation console panels incl. the appeal-review-before-decide gate, the WS-T comment-flow BFF spec — inline story comments + legacy thread redirect, the WS-K AI provenance-label component, and the WS-R.11 `lcap_v2` schema + §21.2 eviction-order suites) | jsdom / Playwright | `pnpm --filter web test` |
