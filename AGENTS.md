...
write server content (`docs/private-p2p/README.md`).  The WS-S crypto/P2P/UI
plane (MLS/HPKE/AEAD/KDF, Helia/libp2p, the Lamport reducer, sync/rendezvous,
UI/media/migration, the update channel) is the next slice.  See "Implementation
...
| packages/private-p2p | ~3 (WS-S Private P2P rooms foundation: the canonical DAG-CBOR determinism/reject/bomb suite — the P1/P2/P3 properties, the integer-boundary table, the full decode rejection matrix, the §27 cap aborts; the strict private-schema accept/reject + the WS-G contribution-op parity + the §10.4↔§10.5 envelope-AAD field alignment; every §13.2 op-body type validated through the op envelope — ~97% coverage) | node | `pnpm --filter @licio/private-p2p test` |
