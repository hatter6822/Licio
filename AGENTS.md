...
- **No protocol/crypto core enters the INITIAL bundle** — but by two different
  mechanisms, and the distinction matters when one of them breaks.
  `@licio/lcap-p2p` and `@licio/private-p2p` are loaded by DYNAMIC import only,
  which `check:lcap-p2p-split` / `check:private-p2p-split` assert (no static
  value import).  The `@licio/lcap` CORE has no such gate and IS imported
  statically by `apps/web/src/lcap/**`; it stays out of the initial payload
  through route code-splitting, and what enforces that is the measured
  initial-payload budget in `check-bundle-size.ts` — which also FAILS if
  `index.html` preloads either plane chunk, because the separate plane budget
  is only sound while the planes are genuinely lazy.  (They were not: rolldown
  chunk groups capture dependencies recursively, so both planes had absorbed
  `zod`/`@licio/shared`/`@licio/lcap` and rode every first paint while all three
  guards reported green.  `apps/web/vite.config.ts` sets
  `includeDependenciesRecursively: false` to prevent it.)
...
- **No `any` (ABSOLUTE).**  Use `unknown` and narrow.  The only remaining
  occurrences are in `apps/web/src/routeTree.gen.ts`, which is GENERATED by the
  TanStack Router plugin and carries `@ts-nocheck` + `biome-ignore-all`.  (The
  previously documented exception, `packages/shared/src/types/**`, was wrong in
  both directions: that directory contains no `any`, while
  `packages/db/src/similarity.ts` carried one behind a `biome-ignore` — now
...
**Test counts.**  `pnpm test` is the canonical query (≈10620 pass without
the gated integration env; ≈10940 with live Postgres/Redis — the gated env
