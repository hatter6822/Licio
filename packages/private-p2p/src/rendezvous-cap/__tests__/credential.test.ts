...
describe('Tier-2 rendezvous-cap credential', () => {
  ...
  it('full flow: request → blind-issue → prove → server verifies + dedups', () => {
    ...
    const { proof, pseudonym } = proveRendezvousPresence(issuer.pk, cred, epoch, ctx);
    expect(verifyRendezvousPresence(issuer.pk, proof, pseudonym, epoch, ctx)).toBe(true);
  ...
  it('two devices in the same bucket get two distinct slots (cap is per-device)', () => {
    const issuer = generateIssuerKeyPair();
    const ctx = rendezvousContext(roomBlindId, epoch, 3);
    const a = proveRendezvousPresence(issuer.pk, enroll(issuer, epoch), epoch, ctx);
    const b = proveRendezvousPresence(issuer.pk, enroll(issuer, epoch), epoch, ctx);
    expect(hex(pseudonymToBytes(a.pseudonym))).not.toBe(hex(pseudonymToBytes(b.pseudonym)));
    expect(verifyRendezvousPresence(issuer.pk, a.proof, a.pseudonym, epoch, ctx)).toBe(true);
    expect(verifyRendezvousPresence(issuer.pk, b.proof, b.pseudonym, epoch, ctx)).toBe(true);
  });
  ...
  it('revocation: a credential for one epoch does not verify in another (epoch rotation)', () => {
    ...
    const presence = proveRendezvousPresence(issuer.pk, cred, newEpoch, ctx);
    expect(
      verifyRendezvousPresence(issuer.pk, presence.proof, presence.pseudonym, newEpoch, ctx),
    ).toBe(false);
  });
  ...
  it('non-member / wrong-issuer shows are rejected', () => {
    ...
    expect(verifyRendezvousPresence(otherIssuer.pk, proof, pseudonym, epoch, ctx)).toBe(false);
