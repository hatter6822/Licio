...
//
// Per-actor: a(u) = wD·dwell(u) + wS·source(u) + wC·context(u) ∈ [0, 1].
...
export function validateActiveAttentionConfig(config: ActiveAttentionConfig): void {
  const { dwellPct, sourcePct, contextPct } = config.weights;
  const parts = [dwellPct, sourcePct, contextPct];
  if (!parts.every((p) => Number.isInteger(p) && p >= 1 && p <= 50)) {
    throw new Error('active-attention weights must be integer percents in [1, 50]');
  }
  if (dwellPct + sourcePct + contextPct !== 100) {
...
export function actorActiveAttention(
  actor: Pick<
    ActorItemSummary,
    'dwellBucket' | 'sourceOpened' | 'sourceBounceOnly' | 'contextOpened'
  >,
  ...
  return clamp01(
    (dwellPct / 100) * dwell + (sourcePct / 100) * source + (contextPct / 100) * context,
  );
}

/**
 * The item's ActiveAttention score in [0, 1) for a window: the saturating sum
 * of bounded per-actor contributions.
 */
export function itemActiveAttention(
  ...
  let sum = 0;
  for (const actor of actors) sum += actorActiveAttention(actor, config);
